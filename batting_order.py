...
def get_play_by_play(game_page_url):
    """
...
def html2pd(pbp_table):
    ...
    The input is a signle game's html table.
    """
    column_flag = 1
    column_list = []
    pbp_list = []
    ...
    for i in range(len(batter_list)):
        if i == 0:
            ...
            if batter_list[i] != batter_list[i - 1]:
                ...
                pa_list[i - 1] = 0
                pa_list.append(1)
    # Check if the last play is incurred by the runner on bases
    last_play = team_df["Play Description"].values[-1].lower()
    if last_play == "picked off" or last_play == "caught stealing":
        pa_list[-1] = 0
    team_df["PA"] = pa_list  # Add column PA to df
    # Filter out non-PA-related actions
    team_df_PA = team_df[team_df["PA"] == 1]
...
def get_team_pa(team_page_url):
    """
    This function is to get the desired info for all 162 games of one team.
    The input is a single teams' page for a single season.
    """
    ...
    for game_page in game_pages:
        ...
        team_df = get_pa(game_page, team)  # Get df for a single game
